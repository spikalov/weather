...
class CRest
{
	...
	/**
	 * Вызов curl для запроса в BX
	 * @var $arParams array
	 * $arParams = [
	 *      'method'    => 'метод api',
	 *      'params'    => []// массив параметров метода
	 * ];
	...
	protected static function callCurl($arParams)
	{
		...
		if ($arSettings !== false) {
			if (isset($arParams['this_auth']) && $arParams['this_auth'] == 'Y') {
				...
				$url = $arSettings["client_endpoint"] . $arParams['method'];
				if (empty($arSettings['is_web_hook']) || $arSettings['is_web_hook'] != 'Y') {
					$arParams['params']['auth'] = $arSettings['access_token'];
				}

				if (!empty($_REQUEST['AUTH_ID']) && ($arParams['method'] == 'profile' || $arParams['method'] == 'user.current')) {
					$arParams['params']['auth'] = $_REQUEST['AUTH_ID'];
				}
			}
